    ...
    StyledListView {
        ...
        model: ScriptModel {
            ...
            values: {
                ...
                // 1. Masukkan Aplikasi yang di-PIN (Wajib Muncul)
                const pinnedApps = Config.options?.dock.pinnedApps ?? [];
                for (const appId of pinnedApps) {
                    // Gunakan toLowerCase agar tidak duplikat karena huruf besar/kecil
                    const key = appId.toLowerCase();
                    if (!map.has(key)) {
                        map.set(key, {
                            appId: appId, // Simpan nama asli untuk pencarian icon
                            pinned: true,
                            toplevels: []
                        });
                    }
                ...
                // Separator (Pemisah antara Pin dan Open Apps) - Opsional
                // if (pinnedApps.length > 0) {
                //     map.set("SEPARATOR", { appId: "SEPARATOR", pinned: false, toplevels: [] });
                // }

                // 2. Masukkan Aplikasi yang SEDANG DIBUKA (Open Windows)
                // Ambil daftar semua jendela dari Hyprland
                const openWindows = ToplevelManager.toplevels.values;
                
                for (const toplevel of openWindows) {
                    // Filter aplikasi kosong/invalid
                    if (!toplevel.appId || toplevel.appId === "") continue;

                    const key = toplevel.appId.toLowerCase();

                    // Jika belum ada di map (belum di-pin), tambahkan sebagai aplikasi baru
                    if (!map.has(key)) {
                        map.set(key, {
                    ...
                    // Masukkan window ini ke daftar toplevels aplikasinya
                    map.get(key).toplevels.push(toplevel);
                }

                // 3. Konversi Map ke Array untuk ditampilkan
                var finalValues = [];
                for (const [key, value] of map) {
                    // Hanya tampilkan jika aplikasi itu DI-PIN atau MEMILIKI WINDOW TERBUKA
                    if (value.pinned || value.toplevels.length > 0) {
                        finalValues.push({ 
                            appId: value.appId, 
